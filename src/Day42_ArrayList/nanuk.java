...
/*
In the Eskimo language `nanuk` means _polar bear._ When the tribe men goes hunting they need to select a leader, each men boasts on how many fish, elk or other animals he can hunt.

Then they vote who will lead them.

A successful hunt for the leader is when they managed to hunt _as much as he boasted_ and the hunting party managed to avoid a polar bear (_nanuk_) which can ruin a whole hunt.
...
public class nanuk {
    ...
    public static boolean nanuk(ArrayList<String> r, int way_stones, int boast) {
        ...
        if (nCount <= way_stones && sum >= boast){
            return true;
        }else {
            return false;
        }
